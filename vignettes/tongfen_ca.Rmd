...
vignette: >
  %\VignetteIndexEntry{TongFen Demo}
...
data_2011 <- tongfen_ca_census_ct(data_2006,data %>% filter(Year=="2011"),"Population")
data_2016 <- tongfen_ca_census_ct(data_2006,data %>% filter(Year=="2016"),"Population")
...
rent_data <- get_tongfen_ca_census_ct(regions=regions,vectors=rent_variables,geo_base = 'CA16')
```
...
vsb <- aggregate_data_with_meta(vsb,bind_rows(meta,tibble(variable=c("Population","Dwellings","Households")))) %>%
